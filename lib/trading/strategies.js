/**
 * Artery Wealth Builder Trading Strategies
 * Implements Momentum, Mean Reversion, and Breakout strategies
 */

const { calculateIndicators, ema } = require('./indicators');

const MONITORED_SYMBOLS = ['ETH-USD', 'BTC-USD'];
const STRATEGY_SYMBOLS = new Set(MONITORED_SYMBOLS);

// Symbols excluded from all automated trading (delisted, illiquid, or manually managed)
const EXCLUDED_SYMBOLS = new Set(['GALA-USD', 'GALA']);
...
async function getMomentumSignal(symbol, candles) {
  ...
  if (emaBullish && rsiHealthy && macdPositive && aboveLongTrend) {
    ...
    if (last.macdHist > 0) confidence += 0.08;
...
async function getMeanReversionSignal(symbol, candles) {
  ...
  // Entry: oversold + at lower BB + significant drop (thresholds relaxed for earlier entries)
  const oversold = last.rsi < 38;
  const atLowerBB = last.close <= last.bbLower * 1.02; // within 2% of lower band
  const significantDrop = change24h < -8;

  if (oversold && atLowerBB && significantDrop) {
    let confidence = 0.60;
    if (last.rsi < 28) confidence += 0.18;
    if (change24h < -18) confidence += 0.12;
...
// Fetch daily candles and return whether price is above the 20-day EMA (bullish regime)
async function getDailyTrend(symbol, coinbaseClient) {
  try {
    const dailyCandles = await coinbaseClient.getCandles(symbol, 'ONE_DAY', 50);
    if (!dailyCandles || dailyCandles.length < 22) return { bullish: true };
    const indicators = calculateIndicators(dailyCandles);
    const last = indicators[indicators.length - 1];
    return { bullish: last.close > last.ema20, ema20: last.ema20 };
  } catch {
    return { bullish: true }; // default to allowing trades on error
  }
}

// Per-symbol strategy bias: ETH dip-buys are more reliable; BTC trends more cleanly
function applySymbolBias(sig) {
  if (sig.signal !== 'BUY') return sig;

  if (sig.symbol === 'ETH-USD') {
    if (sig.strategy === 'MEAN_REVERSION') return { ...sig, confidence: Math.min(0.95, sig.confidence + 0.05) };
    if (sig.strategy === 'BREAKOUT')       return { ...sig, confidence: Math.max(0,    sig.confidence - 0.05) };
  }

  if (sig.symbol === 'BTC-USD') {
    if (sig.strategy === 'MOMENTUM')       return { ...sig, confidence: Math.min(0.95, sig.confidence + 0.05) };
    if (sig.strategy === 'MEAN_REVERSION') return { ...sig, confidence: Math.max(0,    sig.confidence - 0.05) };
  }

  return sig;
}

// Adjust BUY signal confidence based on daily trend regime
function applyDailyTrendFilter(sig, dailyTrend) {
  if (sig.signal !== 'BUY') return sig;
  if (dailyTrend.bullish) {
    // Small bonus for trading with the daily trend
    return { ...sig, confidence: Math.min(0.95, sig.confidence + 0.03) };
  }
  // Penalty for trading against daily trend; lighter for mean reversion (buys dips)
  const penalty = sig.strategy === 'MEAN_REVERSION' ? 0.05 : 0.10;
  return {
    ...sig,
    confidence: Math.max(0, sig.confidence - penalty),
    reason: sig.reason + ' (daily bear)',
  };
}

/**
 * Generate signals for all monitored assets plus any additional held symbols
 * @param {object} coinbaseClient
 * @param {string[]} heldSymbols - extra symbols derived from live portfolio holdings
 */
async function getAllSignals(coinbaseClient, heldSymbols = []) {
  const STABLECOINS = new Set(['USD', 'USDC', 'USDT', 'DAI', 'BUSD']);
  const extra = heldSymbols
    .map(s => s.includes('-') ? s : `${s}-USD`)
    .filter(s => !STABLECOINS.has(s.replace('-USD', '')) && !MONITORED_SYMBOLS.includes(s) && !EXCLUDED_SYMBOLS.has(s));
  const symbols = [...MONITORED_SYMBOLS, ...extra];

  const signals = [];

  for (const symbol of symbols) {
    const isHeld     = extra.includes(symbol);
    const isStrategy = STRATEGY_SYMBOLS.has(symbol);
    try {
      // Fetch 1h candles and daily trend in parallel
      const [candles, dailyTrend] = await Promise.all([
        coinbaseClient.getCandles(symbol, 'ONE_HOUR', 200),
        getDailyTrend(symbol, coinbaseClient),
      ]);
      ...
      // Apply symbol bias then daily trend filter to each signal
      const adjusted = [momentum, meanReversion, breakout]
        .map(sig => applySymbolBias(sig))
        .map(sig => applyDailyTrendFilter(sig, dailyTrend));

      // Add all actionable signals
      adjusted.forEach(sig => {
        if (sig.signal !== 'HOLD' && sig.confidence > 0.5) {
          signals.push({ ...sig, held: isHeld, strategy_signal: isStrategy });
        }
      });

      // If no strong signal, add best HOLD for status display
      if (!adjusted.some(s => s.signal !== 'HOLD')) {
        const best = adjusted.sort((a, b) => b.confidence - a.confidence)[0];
        signals.push({ ...best, held: isHeld, strategy_signal: isStrategy });
      }
    } catch (error) {
      console.error(`Error getting signals for ${symbol}:`, error.message);
      signals.push({
        symbol, signal: 'HOLD', confidence: 0, strategy: 'ERROR',
        price: 0, reason: error.message, held: isHeld, strategy_signal: isStrategy,
      });
    }
  }
