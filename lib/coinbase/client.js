...
class CoinbaseClient {
  ...
  async placeOrder(symbol, side, size, orderType = 'market', limitPrice = null) {
    ...
    //   BUY  → quote_size  (how many USD to spend)
  ...
  // Place a stop-limit SELL order on Coinbase — enforced at exchange level.
  // stopPrice:  price at which the order activates (e.g. entry * 0.97)
  // limitPrice: the actual sell limit once activated (slightly below stop to ensure fill)
  async placeStopLimitSell(symbol, baseSize, stopPrice, limitPrice) {
    return this.request('POST', '/orders', {
      client_order_id: `artery-stop-${Date.now()}-${randomBytes(4).toString('hex')}`,
      product_id:      symbol,
      side:            'SELL',
      order_configuration: {
