...
function importPrivateKey(secret) {
  ...
  if (!raw.includes('-----BEGIN')) {
    ...
    // Try P-256 / ES256 first (Coinbase docs specify ES256 for CDP keys)
    try {
      const sec1 = Buffer.concat([SEC1_P256_PREFIX, scalar, SEC1_P256_SUFFIX]);
      const key = createPrivateKey({ key: sec1, format: 'der', type: 'sec1' });
      return { key, isEd25519: false };
    } catch (_) { /* fall through to Ed25519 */ }

    // Fall back to Ed25519 / EdDSA
