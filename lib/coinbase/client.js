...
// PKCS#8 DER prefix for Ed25519 (wraps a raw 32-byte seed into a Node-importable key)
...
function importPrivateKey(secret) {
  ...
  if (!raw.includes('-----BEGIN')) {
    // Raw base64 Ed25519 seed (Coinbase CDP JSON format: { "privateKey": "<base64>" })
    const keyBytes = Buffer.from(raw, 'base64');
    // CDP gives 64 bytes (seed + pubkey); Ed25519 seed is the first 32 bytes
    const seed = keyBytes.length >= 64 ? keyBytes.slice(0, 32) : keyBytes;
    const pkcs8 = Buffer.concat([ED25519_PKCS8_PREFIX, seed]);
    return { key: createPrivateKey({ key: pkcs8, format: 'der', type: 'pkcs8' }), isEd25519: true };
  }
