  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #EBEBEA', borderRadius: 8, padding: '8px 12px', fontSize: 11, boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}>
      ...
      <div style={{ color: pnl >= 0 ? '#16A34A' : '#DC2626' }}>
        {pnl >= 0 ? '+' : ''}${pnl?.toFixed(2)} ({((pnl / initialCapital) * 100).toFixed(2)}%)
      </div>
    </div>
  );
}

export default function LiveChart({ snapshots = [], initialCapital }) {
  const [chartData, setChartData] = useState([]);

  useEffect(() => {
    if (snapshots.length === 0) {
      const now = new Date();
      const seed = initialCapital || 0;
      const placeholder = Array.from({ length: 14 }, (_, i) => {
        const date = new Date(now);
        date.setDate(date.getDate() - (13 - i));
        return {
          date: date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
          value: seed + (Math.random() - 0.3) * (seed * 0.07),
        };
      });
      placeholder[placeholder.length - 1].value = seed;
  ...
  const baseline = initialCapital || chartData[0]?.value || 0;
  const target = baseline * 1.15;
  const currentValue = chartData[chartData.length - 1]?.value || baseline;
  const pnl = currentValue - baseline;
  const pnlPct = baseline ? (pnl / baseline) * 100 : 0;
  const isPositive = pnl >= 0;

  const minValue = Math.min(...chartData.map(d => d.value), baseline) * 0.98;
  const maxValue = Math.max(...chartData.map(d => d.value), baseline) * 1.02;
      ...
      <ResponsiveContainer width="100%" height={220}>
        <AreaChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
          ...
          <YAxis
            ...
            width={55}
          />
          <Tooltip content={<CustomTooltip initialCapital={baseline} />} />
          <ReferenceLine
            y={baseline}
            stroke="#EBEBEA"
            strokeDasharray="4 4"
